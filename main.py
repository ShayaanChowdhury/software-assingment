...
def open_main_window():
    ...
    button2 = tk.Button(root, text="Start Quiz", height=3, width=15, bg='gray20', fg='white', relief="raised", command=open_quiz)
...
def open_quiz_window():
    ...
    label_quiz = tk.Label(quiz_window, text="Quiz", font=("Impact", 45), bg='navajo white', padx=20, pady=10, borderwidth=2)
    label_quiz.place(x=430, y=0)

    button_next = tk.Button(quiz_window, text="Next Question", height=3, width=15, bg='gray20', fg='white', relief="raised")
    button_next.place(x=215, y=460)

    button_exit = tk.Button(quiz_window, text="Exit", height=3, width=15, bg='gray20', fg='white', relief="raised", command=quiz_window.destroy)
    button_exit.place(x=50, y=460)

    img_5 = ImageTk.PhotoImage(Image.open("Star.png").resize((100, 100)))
    Label21 = tk.Label(quiz_window, image=img_5, bg='navajo white')
    Label21.image = img_5
    Label21.place(x=300, y=0)

    img_6 = ImageTk.PhotoImage(Image.open("Star.png").resize((100, 100)))
    Label22 = tk.Label(quiz_window, image=img_6, bg='navajo white')
    Label22.image = img_6
    ...
    def load_question(question_data):
        ...
        if options:
            ...
            for i, option in enumerate(options):
                btn = tk.Radiobutton(quiz_window, text=option, font=('Comic Sans Ms', 15), value=option, variable=r, bg='navajo white')
                btn.place(x=60, y=200 + i * 50)
                option_buttons.append(btn)
        else:
            answer_entry = tk.Entry(quiz_window, font=("Comic Sans MS", 15), width=30)
        ...
        if "image" in question_data:
            ...
            question_image_label = tk.Label(quiz_window, image=question_image, bg='navajo white')
    ...
    r = tk.StringVar()
    label_question = tk.Label(quiz_window, text="", font=("Comic Sans MS", 17), bg='navajo white', padx=20, pady=10, borderwidth=2)
    ...
    def show_final_score():
        ...
        button_backhome = tk.Button(final_screen, text="Back Home", height=3, width=15, bg='gray20', fg='white', relief="raised", command=back_to_home)
        button_backhome.place(x=10, y=475)

        button_exit = tk.Button(final_screen, text="Exit", height=3, width=15, bg='gray20', fg='white', relief="raised", command=quiz_window.destroy)
